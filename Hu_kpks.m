...
for k=1:1:Np-1;
    ...
    for jj=0:1:s-1;
        ...
        for m=0:1:k-1;
            spjm1 = spjm1 + ( ( (fac(Np+1) ) / (fac(m+1) * fac(Np-m+1) ) ) .* (pj.^m) .* ((1-pj).^(Np-m)) );
            %spjm1 = ( ( (fac(Np+1) ) / (fac(m+1) * fac(Np-m+1) ) ) .* (pj.^m) .* ((1-pj).^(Np-m)) );
            %s1(jj+1,m+1)=spjm1;
        end
            spjm2 = spjm1 + ( ( (fac(Np+1) ) / (fac(k+1) * fac(Np-k+1) ) ) .* (pj.^k) .* ((1-pj).^(Np-k)) );        
    ...
    for jj=0:1:s-1;
        ...
        for m=0:1:k;
            spjm2 = spjm2 + ( ( (fac(Np+1) ) / (fac(m+1) * fac(Np-m+1) ) ) .* (pj.^m) .* ((1-pj).^(Np-m)) );
            %spjm2(m+1) = ( ( (fac(Np+1) ) / (fac(m+1) * fac(Np-m+1) ) ) .* (pj.^m) .* ((1-pj).^(Np-m)) );
            %s2(jj+1,m+1)=spjm2;
        end
